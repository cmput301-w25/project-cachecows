...
public class FeedManagerRecentFilterTest {
    ...
    private void seedTestData() throws ExecutionException, InterruptedException {
        ...
        // Create 4 mood events with staggered timestamps
    ...
    public void testThreeMostRecentFilter() {
        ...
        // Open filter menu and select 3 most recent
        onView(withId(R.id.btnFilter)).perform(click());

        // CORRECTED: Match the exact text from view hierarchy
        onView(withText("Show 3 Most Recent")).perform(click());

        // Wait for filter application
        try { Thread.sleep(2000); } catch (InterruptedException e) {}

        // Verify only 3 most recent moods are shown
